...
function runValidation(res, name, student_no, email, phone) {
  ...
  if (!student_no || !/^\d+$/.test(student_no)) {
    return renderFormPage(
      res,
      "Student no is required and must be in number format"
    );
  }
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    return renderFormPage(
      res,
      "Email no is required and must be in email format"
    );
  }
  if (!/^\d+$/.test(phone)) {
...
router.post("/update/:id", async (req, res) => {
  ...
  try {
    ...
    renderFormPage(
      res,
      "Database error. Failed to update Students. Please check DB"
    );
  }
});
